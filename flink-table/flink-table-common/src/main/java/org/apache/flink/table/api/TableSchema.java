...
public class TableSchema {
    ...
    /**
     * 将此模式的所有列转换为(可能嵌套的)行数据类型。
     *
     * <p>注意:返回的行数据类型同时包含物理列和计算列。在表源或表接收器中使用此方法时要小心。在很多情况下，
     * {@link #toPhysicalRowDataType()}可能更合适。
     *
    ...
    /**
     * 从 {@link TypeInformation} 实例创建表模式。如果类型信息是 {@link CompositeType}，则组合类型的字段名和
     * 类型用于构造 {@link TableSchema} 实例。否则，将创建带有单个字段的表模式。字段名是“f0”，字段类型是提供的类型。
     *
    ...
    /**
     * 验证字段名 {@code fieldNames} 和字段类型 {@code fieldTypes} 具有相同的数量。
     *
