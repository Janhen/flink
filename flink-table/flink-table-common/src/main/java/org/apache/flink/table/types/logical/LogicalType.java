...
/**
 * 描述值的数据类型的逻辑类型。它并不意味着传输或存储的具体物理表示，而是定义了基于 jvm 的语言和表生态系统之间的边界。
 *
 * <p> 逻辑类型的定义类似于 SQL 标准的“数据类型”术语，但也包含了关于值的可空性的信息，以便有效地处理标量表达式
 *
 * <p> 这个类的子类定义内置或用户定义类型的特征。每个逻辑类型都必须支持可空性。
 *
 * <p> 这个类的实例描述了完全参数化的、不可变的类型，带有额外的信息，如数字精度或期望长度。
 *
 * <p> 通过 {@link LogicalTypeCasts} 和 {@link LogicalTypeGeneralization} 定义逻辑类型如何与其他类型相关。
 *
 * <p> 注意：逻辑类型只是类型的描述，规划器或运行时可能不支持每种逻辑精度的每种类型！
 *
