 ...
 * 与 SQL 标准相比，不支持闰秒(23:59:60 和 23:59:61)，因为其语义更接近 {@link java.time.OffsetDateTime}。
 *
 * <p>序列化的字符串表示形式是 {@code TIMESTAMP(p) WITH TIME ZONE}，其中 {@code p} 是小数秒的位数(=精度)。
 *   {@code p} 的值必须在 0 到 9 之间(包括两者)。如果没有指定精度，{@code p} 等于 6。
 *
 * <p>与 {@link LocalZonedTimestampType} 相比，时区偏差信息物理地存储在每个数据中。它单独用于每一个计算、可视化或
 *   与外部系统的通信。
 *
 * <p>从 {@link java.time.ZonedDateTime} 忽略 zone ID。
 *
