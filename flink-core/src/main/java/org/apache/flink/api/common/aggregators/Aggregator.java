...
/**
 * 聚合器是一种跨函数的并行实例聚合值的方法。聚合器收集关于在函数中执行的实际工作的简单统计信息(例如处理元素的数量)。聚合
 ...
 *
 * <p>聚合器必须在通过函数使用它们的迭代处注册。在Java API中，当将聚合器和收敛准则一起使用时，方法是
 ...
 *
 * <p>聚合器必须<i>分布式<i>：聚合器必须能够预聚合值，并且必须能够聚合这些预聚合值以形成最终聚合。许多聚合函数都满足这
