...
/**
 * 对配置对象的读取访问权限。允许读取用包含在 {@link ConfigOption} 中的元信息描述的值
 *
...
public interface ReadableConfig {
    ...
    /**
     * 使用 {@link ConfigOption} 中包含的元数据读取值。如果配置中不存在值键，则返回 {@link ConfigOption#defaultValue()}
     *
    ...
    /**
     * 使用 {@link ConfigOption} 中包含的元数据读取值。与 {@link #get(ConfigOption)} 相反，如果值不存在，则返
     * 回 {@link Optional#empty()}
     *
